...
contract OracleVerifier {
    ...
    event OutcomeReceived(uint256 indexed requestId, address indexed submitter, uint256 matrixLenght);

    // This is the function called by the oracles in GO
    function saveOutcome(uint256 requestId, int128[] calldata _flatMatrix) external {
        // If it has already been processed, stop in order to spare gas
        //if (processedRequests[requestId]) {
        require(!results[requestId].saved, "Request already fulfilled");
        ...
        emit OutcomeReceived(requestId, msg.sender,_flatMatrix.length);
    }

    // Getter to retrieve the matrix (for the smart licenses)
    function getResult(uint256 _requestId) external view returns (int128[] memory, address, uint256) {
        require(results[_requestId].saved, "Result not found");

        Result memory r = results[_requestId];
        return (r.flatMatrix, r.submitter, r.timestamp);
    }

    // Check if the job is already completed (for the transmitter)
    function isCompleted(uint256 _requestId) external view returns (bool) {
        return results[_requestId].saved;
    }
